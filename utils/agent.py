...
class Agent():
    ...
    """
    def __init__(self, n_actions, device, hidden=128, learning_rate=0.0000625,
        ...
        # Create 2 models
        self.main_dqn = DQN(self.n_actions, self.hidden)
        self.target_dqn = DQN(self.n_actions, self.hidden)
...
class Action_Scheduler():
    ...
    def __init__(self, num_actions, model, eps_initial=1, eps_final=0.1, eps_final_frame=0.01,
                eps_annealing_frames=1000000, replay_memory_start_size=50000, 
                max_frames=25000000):
        """
        Args:
    ...
    def get_action(self, frame_number, state, evaluation=False):
        ...
        if len(state) < 4:
            ...
            state_stack = torch.stack(list(state))
        
        return self.model.predict_action(state_stack.unsqueeze(0)).item()